...
def playlist_add(ip, port, name, file, channels_dict):
...
def channel_checker(sock, arguments):
    ...
    try:
        ready = select.select([sock], [], [], arguments.info_timeout)
        if ready[0]:
            sock.close()
            return True
        else:
...
def get_ffprobe(address, port, arguments):
    """ To get the json data from ip:port """
    try:
        result = subprocess.run(
            ['ffprobe', '-v', 'quiet', '-print_format', 'json', '-show_programs', f'udp://{address}:{port}'],
            capture_output=True, text=True, timeout=arguments.info_timeout)
        json_string = json.loads(str(result.stdout))
        channel_name = json_string['programs'][0]['tags']['service_name']
    ...
    for ip in ip_addresses:
        for port in ports:
            ...
            if result:
                ...
                channel_name = get_ffprobe(ip, port, args)
                if channel_name:
...
if ip_list.is_multicast:
    ...
    print(f'[*] IPs provided are not multicast. Please try again.')
    sys.exit()
...
try:
    ...
    print(f'[*] ERROR: the number of threads must be an integer.\n')
    sys.exit()
